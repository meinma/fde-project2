...
val pickupLatitudeBucket = trips5.withColumn("pickupLat",floor(ceil($"pickup_latitude" * 111111 ) / (2 * dist))).withColumn("pickupLon",floor(ceil($"pickup_longitude"*111111) / (2 * dist)))
...
val dropoffLatitude = trips5.withColumn("dropoffLat",floor(ceil($"dropoff_latitude" * 111111 ) / (2 * dist))).withColumn("dropoffLon",floor(ceil($"dropoff_longitude" * 111111)/(2 * dist)))
